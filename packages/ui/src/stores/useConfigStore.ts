...
const fetchOcelotDefaults = async (): Promise<OcelotDefaults> => {
    markStartupTrace('config.defaults:start');
    const started = typeof performance !== 'undefined' ? performance.now() : Date.now();
    const finish = (source: string, result: OcelotDefaults) => {
...
interface ConfigStore {
    ...
    // Ocelot settings-based defaults (take precedence over agent preferences)
        ...
        persist(
            (set, get) => ({
                ...
                loadAgents: async (options) => {
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        try {
                            // Fetch agents and Ocelot settings in parallel. OpenCode config
                            ...
                            // Detect invalid Ocelot settings so we can clear them from storage.
                ...
                setAgent: (agentName: string | undefined) => {
                    ...
                    if (agentName) {
                        ...
                        if (currentSessionId && useSessionUIStore.getState().isOcelotCreatedSession(currentSessionId)) {
                            const existingAgentModel = selState.getAgentModelForSession(currentSessionId, agentName);
                            if (!existingAgentModel) {
                                useSessionUIStore.getState().initializeNewOcelotSession(currentSessionId, agents);
                            }
                        }
                    }
