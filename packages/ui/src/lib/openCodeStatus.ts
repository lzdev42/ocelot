...
const buildOpenCodeStatusReport = async (): Promise<string> => {
  ...
  const openChamberHealth: OcelotHealthSnapshot | null = await (async () => {
    ...
    try {
      ...
      return json as OcelotHealthSnapshot;
    } catch {
      return null;
    } finally {
      clearTimeout(timeout);
  ...
  const openChamberOpencodeResolutionResult: {
    data: OcelotOpencodeResolution | null;
    status: number | null;
    error: string | null;
  } = await (async () => {
    ...
    try {
      ...
      return { data: json as OcelotOpencodeResolution, status: resp.status, error: null };
    } catch (error) {
      return {
        data: null,
        status: null,
  ...
  lines.push(`Ocelot version: ${appVersion}`);
