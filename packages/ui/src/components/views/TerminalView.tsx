...
export const TerminalView: React.FC = () => {
    ...
    const handleCdToConfigDir = React.useCallback(() => {
        if (isReconnectPending) return;
        const terminalId = terminalIdRef.current;
        if (!terminalId) return;
        void terminal.sendInput(terminalId, 'cd ~/.config/opencode\r').catch((error) => {
            if (!isReconnectPending) {
                setConnectionError(
                    error instanceof Error ? error.message : t('terminalView.error.sendInputFailed')
                );
        ...
        focusTerminalController();
    }, [focusTerminalController, isReconnectPending, t, terminal]);
                                    ...
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                            <Button
                                                ...
                                                size="xs"
                                                variant="ghost"
                                                onClick={handleCdToConfigDir}
                                                className={cn('shrink-0 p-0', isMobile ? 'h-8 w-8' : 'h-7 w-7')}
                                                aria-label={t('terminalView.bottomDock.cdToConfigDirAria')}
                                            >
                                                <Icon name="folder-open" className="h-4 w-4" />
                                            </Button>
                                        </TooltipTrigger>
                                        <TooltipContent>
                                            <p>{t('terminalView.bottomDock.cdToConfigDirTitle')}</p>
                                        </TooltipContent>
                                    </Tooltip>
                                    <Button
                                        type="button"
