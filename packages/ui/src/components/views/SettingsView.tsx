...
import { AirouterPage } from '@/components/sections/airouter/AirouterPage';
import type { OcelotSection } from '@/components/sections/openchamber/types';
import { OcelotPage } from '@/components/sections/openchamber/OcelotPage';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ onClose, forceMobile, isWindowed, visiblePageSlugs, initialMobileStage = 'nav' }) => {
  ...
  const openChamberSectionBySlug: Partial<Record<SettingsPageSlug, OcelotSection>> = React.useMemo(() => ({
    appearance: 'visual',
    chat: 'chat',
    shortcuts: 'shortcuts',
    sessions: 'sessions',
  ...
  const getPageTitle = React.useCallback((slug: SettingsPageSlug): string => {
    switch (slug) {
      ...
      case 'airouter':
        return t('settings.page.airouter.title');
      case 'about':
  ...
  const renderPageContent = React.useCallback((slug: SettingsPageSlug) => {
    ...
    switch (slug) {
      ...
      case 'tunnel': {
        ...
        return <OcelotPage section={section} />;
      }
      default:
        return <SettingsHome onOpen={openPage} />;
    }
