...
type CommandSource = 'ocelot' | 'opencode' | 'skill';
...
export const CommandAutocomplete = React.forwardRef<CommandAutocompleteHandle, CommandAutocompleteProps>(({
      ...
      try {
        ...
        const builtInCommands: CommandInfo[] = [
          ...(hasSession && !hasMessagesInCurrentSession
            ? [{ id: 'ocelot:init', name: 'init', source: 'ocelot' as const, description: t('chat.commandAutocomplete.command.initDescription'), isBuiltIn: true }]
            : []
          ),
          ...(hasSession  // Show when session exists, not when hasMessages
            ? [
                { id: 'ocelot:undo', name: 'undo', source: 'ocelot' as const, description: t('chat.commandAutocomplete.command.undoDescription'), isBuiltIn: true },
                { id: 'ocelot:redo', name: 'redo', source: 'ocelot' as const, description: t('chat.commandAutocomplete.command.redoDescription'), isBuiltIn: true },
                { id: 'ocelot:timeline', name: 'timeline', source: 'ocelot' as const, description: t('chat.commandAutocomplete.command.timelineDescription'), isBuiltIn: true },
              ]
            : []
          ),
          { id: 'ocelot:compact', name: 'compact', source: 'ocelot' as const, description: t('chat.commandAutocomplete.command.compactDescription'), isBuiltIn: true },
          ...(hasSession
            ? [{ id: 'ocelot:summary', name: 'summary', source: 'ocelot' as const, description: t('chat.commandAutocomplete.command.summaryDescription'), isOcelot: true }]
            : []
          ),
          ...(canStartSessionCommand
            ? [{ id: 'ocelot:workspace-review', name: 'workspace-review', source: 'ocelot' as const, description: t('chat.commandAutocomplete.command.workspaceReviewDescription'), isOcelot: true }]
            : []
          ),
          ...(canUseReviewHandoffFlow
            ? [{ id: 'ocelot:handoff-review', name: 'handoff-review', source: 'ocelot' as const, description: t('chat.commandAutocomplete.command.handoffReviewDescription'), isOcelot: true }]
            : []
          ),
          ...(canStartSessionCommand
            ? [{ id: 'ocelot:plan-feature', name: 'plan-feature', source: 'ocelot' as const, description: t('chat.commandAutocomplete.command.featurePlanDescription'), isOcelot: true }]
            : []
          ),
          ...(canStartSessionCommand
            ? [{ id: 'ocelot:catch-up', name: 'catch-up', source: 'ocelot' as const, description: t('chat.commandAutocomplete.command.catchUpDescription'), isOcelot: true }]
            : []
          ),
          ...(canStartSessionCommand
            ? [{ id: 'ocelot:debug', name: 'debug', source: 'ocelot' as const, description: t('chat.commandAutocomplete.command.debugDescription'), isOcelot: true }]
            : []
          ),
          ...(canStartSessionCommand
            ? [{ id: 'ocelot:weigh', name: 'weigh', source: 'ocelot' as const, description: t('chat.commandAutocomplete.command.weighDescription'), isOcelot: true }]
            : []
          ),
          ...(canStartSessionCommand
            ? [{ id: 'ocelot:explore', name: 'explore', source: 'ocelot' as const, description: t('chat.commandAutocomplete.command.exploreDescription'), isOcelot: true }]
            : []
          ),
        ];
  ...
  return (
    <div
      ref={containerRef}
      className="absolute z-[100] min-w-0 w-full max-h-64 bg-background border-2 border-border/60 rounded-xl shadow-none bottom-full mb-2 left-0 flex flex-col"
        ...
        {loading ? (
          ...
          <div>
            {commands.map((command, index) => {
              ...
              const isOcelotBadge = command.isOcelot;
              return (
                <div
                  key={command.id}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {command.description && !isMobile && (
                      <div className="typography-meta text-muted-foreground mt-0.5 break-words">
                        {command.description}
                      </div>
                    )}
                  </div>
