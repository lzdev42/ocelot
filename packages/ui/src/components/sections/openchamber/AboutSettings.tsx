...
import { OcelotLogo } from '@/components/ui/OcelotLogo';
...
export const AboutSettings: React.FC<AboutSettingsProps> = ({ initialUpdateDialogOpen = false }) => {
  ...
  const [openChamberVersion, setOcelotVersion] = React.useState<string | null>(null);
    ...
    const loadOcelotVersion = async () => {
      try {
        ...
        const version = typeof data?.openchamberVersion === 'string' && data.openchamberVersion.trim().length > 0
          ...
          : null;
        if (!cancelled) setOcelotVersion(version);
      } catch {
        if (!cancelled) setOcelotVersion(null);
      }
    };

    void loadOcelotVersion();
    ...
    if (updateStore.checking) {
      ...
      const timer = setTimeout(() => {
        ...
        if (didInitiateCheck.current && !updateStore.available && !updateStore.error) {
          toast.success(t('settings.ocelot.about.toast.latestVersion'));
    ...
    return (
      <div className="w-full space-y-6 pb-2">
        <div className="flex flex-col items-center text-center">
          <OcelotLogo width={72} height={72} />
          <h2 className="mt-4 typography-ui-header font-semibold text-foreground">Ocelot</h2>
          <div className="mt-2 space-y-1 typography-ui text-muted-foreground">
            ...
            <p>{t('aboutDialog.openCodeVersionLabel', { version: openCodeVersion || t('settings.ocelot.about.state.unknown') })}</p>
          </div>
        </div>
        ...
        <div className="flex justify-center">
          {!updateStore.available && !updateStore.error && (
            ...
            >
              ...
              {isChecking ? t('settings.ocelot.about.state.checking') : t('settings.ocelot.about.actions.checkForUpdates')}
            </Button>
          )}
          ...
          {!isChecking && updateStore.available && (
            ...
            >
              ...
              {t('settings.ocelot.about.actions.updateToVersion', { version: updateStore.info?.version || '' })}
            </Button>
          )}
        </div>
      ...
      <div className="rounded-lg bg-[var(--surface-elevated)]/70 overflow-hidden flex flex-col">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-4 py-3 border-b border-[var(--surface-subtle)]">
          <div className="flex min-w-0 flex-col">
            <span className="typography-ui-label text-foreground">{t('settings.ocelot.about.field.version')}</span>
            <span className="typography-meta text-muted-foreground font-mono">{currentVersion}</span>
          </div>
          <div className="flex min-w-0 flex-col">
            <span className="typography-ui-label text-foreground">{t('settings.ocelot.about.field.openCodeVersion')}</span>
            <span className="typography-meta text-muted-foreground font-mono">{openCodeVersion || t('settings.ocelot.about.state.unknown')}</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            {updateStore.checking && (
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="typography-meta">{t('settings.ocelot.about.state.checking')}</span>
              </div>
            )}
            ...
            {!updateStore.checking && !updateStore.available && !updateStore.error && (
              <span className="typography-meta text-muted-foreground">{t('settings.ocelot.about.state.upToDate')}</span>
            )}
            ...
            >
              {t('settings.ocelot.about.actions.checkForUpdates')}
            </Button>
          </div>
        </div>
