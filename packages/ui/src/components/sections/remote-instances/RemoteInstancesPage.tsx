...
const phaseLabelKey = (phase?: string): I18nKey => {
  switch (phase) {
    ...
    case 'installing':
      return 'settings.remoteInstances.page.phase.installingOcelot';
    case 'updating':
      return 'settings.remoteInstances.page.phase.updatingOcelot';
...
export const RemoteInstancesPage: React.FC = () => {
  ...
  const createPairingLink = React.useCallback(async () => {
    ...
    try {
      ...
      const payload = buildClientConnectionPayload({ serverUrl, token: result.token, label: remoteClientLabel || 'Ocelot' });
