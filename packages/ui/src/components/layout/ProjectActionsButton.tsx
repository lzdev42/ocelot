...
export const ProjectActionsButton = ({
  ...
  const [actions, setActions] = React.useState<OcelotProjectAction[]>([]);
  ...
  const autoDiscoverAction = React.useMemo<OcelotProjectAction>(() => ({
  ...
  const getOrCreateActionTab = React.useCallback(async (action: OcelotProjectAction, options: { revealTerminal?: boolean } = {}) => {
  ...
  const runAction = React.useCallback(async (action: OcelotProjectAction) => {
    ...
    try {
      const discovered = action.id === AUTO_DISCOVER_ACTION_ID
        ? await (async (): Promise<OcelotProjectAction> => {
  ...
  const stopAction = React.useCallback(async (action: OcelotProjectAction) => {
    const runKey = toProjectActionRunKey(normalizedDirectory, action.id);
  ...
  const handleSelectAction = React.useCallback((action: OcelotProjectAction, toggleStopIfRunning = false) => {
