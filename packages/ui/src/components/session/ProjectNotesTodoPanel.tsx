...
import {
  ...
  type OcelotProjectPlanFileLink,
  type OcelotProjectTodoItem,
  type ProjectRef,
...
type ProjectPlanListItem = OcelotProjectPlanFileLink & {
  title: string;
};

const toPlanListItem = async (
  plan: OcelotProjectPlanFileLink,
...
const sortTodosWithCompletedLast = (items: OcelotProjectTodoItem[]): OcelotProjectTodoItem[] => [
  ...items.filter((todo) => !todo.completed),
  ...items.filter((todo) => todo.completed),
];

const insertTodoBeforeCompleted = (items: OcelotProjectTodoItem[], item: OcelotProjectTodoItem): OcelotProjectTodoItem[] => {
...
export const ProjectNotesTodoPanel: React.FC<ProjectNotesTodoPanelProps> = ({
  ...
  const [todos, setTodos] = React.useState<OcelotProjectTodoItem[]>([]);
  ...
  const initializeNewOcelotSession = useSessionUIStore((state) => state.initializeNewOcelotSession);
  ...
  const persistProjectData = React.useCallback(
    async (nextNotes: string, nextTodos: OcelotProjectTodoItem[]) => {
      if (!projectRef) {
        return false;
      }
  ...
  const handleConfirmSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingSendTarget.kind === 'worktree') {
          ...
          initializeNewOcelotSession(session.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, createSession, initializeNewOcelotSession, onActionComplete, pendingSendTarget, projectRef, routeToChat, sendMessage, setCurrentSession, t]
  );
