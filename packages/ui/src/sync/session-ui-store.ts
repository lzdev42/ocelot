...
export type SessionUIState = {
  ...
  clearError: () => void
  markSessionAsOcelotCreated: (sessionId: string) => void
  isOcelotCreatedSession: (sessionId: string) => boolean
  getContextUsage: (contextLimit: number, outputLimit: number) => SessionContextUsage | null
  initializeNewOcelotSession: (sessionId: string, agents: unknown[]) => void
...
export async function materializeOpenDraftSession(selection: {
  ...
  store.initializeNewOcelotSession(created.id, configState.agents ?? [])
...
export const useSessionUIStore = create<SessionUIState>()((set, get) => ({
  ...
  markSessionAsOcelotCreated: (sessionId) =>
    set((s) => {
  ...
  isOcelotCreatedSession: (sessionId) => get().webUICreatedSessions.has(sessionId),
  ...
  createSessionFromAssistantMessage: async (sourceMessageId, execution) => {
    ...
    if (execution.createWorktree) {
      ...
      if (!project?.path) {
        throw new Error("Project is not registered in Ocelot")
      }
