...
const pwaDevEnabled = process.env.OCELOT_DISABLE_PWA_DEV !== '1';
...
export default defineConfig({
  ...
  resolve: {
    alias: [
      ...
      { find: '@ocelot/ui', replacement: path.resolve(__dirname, '../ui/src') },
  ...
  server: {
    ...
    proxy: {
      '/auth': {
        target: `http://127.0.0.1:${process.env.OCELOT_PORT || 3001}`,
        changeOrigin: true,
      },
      '/health': {
        target: `http://127.0.0.1:${process.env.OCELOT_PORT || 3001}`,
