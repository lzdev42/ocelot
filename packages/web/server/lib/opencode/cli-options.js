export const parseServeCliOptions = ({
  ...
  const envPassword =
    env.OCELOT_UI_PASSWORD ||
    env.OPENCODE_UI_PASSWORD ||
    null;
  const envCfTunnel = env.OCELOT_TRY_CF_TUNNEL === 'true';
  const envTunnelProvider = env.OCELOT_TUNNEL_PROVIDER || undefined;
  const envTunnelMode = env.OCELOT_TUNNEL_MODE || undefined;
  const envTunnelConfigRaw = env.OCELOT_TUNNEL_CONFIG;
  const envTunnelConfig = typeof envTunnelConfigRaw === 'string'
    ? (envTunnelConfigRaw.trim().length > 0 ? envTunnelConfigRaw.trim() : null)
    : undefined;
  const envTunnelToken = env.OCELOT_TUNNEL_TOKEN || undefined;
  const envTunnelHostname = env.OCELOT_TUNNEL_HOSTNAME || undefined;
  const envApiOnly = env.OCELOT_API_ONLY === '1' || env.OCELOT_API_ONLY === 'true';
