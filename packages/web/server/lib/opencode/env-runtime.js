...
export const createOpenCodeEnvRuntime = (deps) => {
  ...
  const bundledOpenCodeCliCandidates = () => {
    ...
    const roots = [
      process.env.OCELOT_BUNDLED_OPENCODE_CLI_DIR,
  ...
  const resolveOpencodeCliPath = (options) => {
    const localOnly = options?.localOnly === true;
    const explicit = [
      process.env.OPENCODE_BINARY,
      process.env.OPENCODE_PATH,
      process.env.OCELOT_OPENCODE_PATH,
      process.env.OCELOT_OPENCODE_BIN,
    ]
    ...
    if (process.platform === 'win32') {
      ...
      return localOnly ? null : bundledOpenCodeCliFallback();
    }
  ...
  const resolveNodeCliPath = () => {
    const explicit = [process.env.NODE_BINARY, process.env.OCELOT_NODE_BINARY]
  ...
  const resolveBunCliPath = () => {
    const explicit = [process.env.BUN_BINARY, process.env.OCELOT_BUN_BINARY]
      .map((v) => (typeof v === 'string' ? v.trim() : ''))
      .filter(Boolean);
  ...
  const ensureOpencodeCliEnv = (options) => {
    const localOnly = options?.localOnly === true;

    if (state.resolvedOpencodeBinary) {
      if (localOnly && state.resolvedOpencodeBinarySource === 'bundled') {
        state.resolvedOpencodeBinary = null;
        state.resolvedOpencodeBinarySource = null;
      } else {
    ...
    const resolved = resolveOpencodeCliPath({ localOnly });
    if (resolved) {
      if (state.useWslForOpencode) {
  ...
  const resolveGitBinaryForSpawn = () => {
    ...
    const explicit = [process.env.GIT_BINARY, process.env.OCELOT_GIT_BINARY]
      .map((value) => (typeof value === 'string' ? value.trim() : ''))
      .filter(Boolean);
    for (const candidate of explicit) {
      if (isExecutable(candidate)) {
