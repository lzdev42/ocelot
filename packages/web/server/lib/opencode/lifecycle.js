...
const HEALTH_CHECK_TIMEOUT_MS = parsePositiveInt(process.env.OCELOT_OPENCODE_HEALTH_TIMEOUT_MS, 5000);
const HEALTH_CHECK_MAX_CONSECUTIVE_FAILURES = parsePositiveInt(
  process.env.OCELOT_OPENCODE_HEALTH_CONSECUTIVE_FAILURES,
  20
);
const HEALTH_CHECK_INTERVAL_OVERRIDE_MS = parsePositiveInt(process.env.OCELOT_OPENCODE_HEALTH_INTERVAL_MS, 0);
const HEALTH_CHECK_RESULT_CACHE_MS = parsePositiveInt(process.env.OCELOT_OPENCODE_HEALTH_CACHE_MS, 750);
...
export const createOpenCodeLifecycleRuntime = (deps) => {
  ...
  const createManagedOpenCodeServerProcess = async ({ hostname, port, timeout, cwd, env: processEnv, shellEnvKeysCount = 0 }) => {
    ...
    // actual host (Electron sets OCELOT_RUNTIME='desktop'; the standalone
    ...
    registerManagedProcess({
      ...
      port,
      binary,
      runtime: process.env.OCELOT_RUNTIME || 'web',
    });
  ...
  const bootstrapOpenCodeAtStartup = async () => {
    try {
      ...
      if (await isOpenCodeProcessHealthy()) {
        ...
        state.openCodeNotReadySince = 0;
        syncToHmrState();
      } else if (process.env.OPENCHAMBER_AWAIT_USER_CHOICE === '1') {
        // Desktop first-launch: do NOT auto-start a managed OpenCode instance.
        // The user must choose a source (local/bundled/remote) and click Start
        // in the onboarding screen; the renderer then calls POST /api/opencode/start.
        console.log('[lifecycle] awaiting user choice before starting OpenCode (OPENCHAMBER_AWAIT_USER_CHOICE=1)');
        // Resolve the OpenCode binary path (without starting it) so /health can
        // report opencodeBinarySource for the onboarding screen's bundled/local
        // availability hints. Non-strict: never throws on a missing/misconfigured
        // binary — the user will pick the source explicitly anyway.
        try {
          await applyOpencodeBinaryFromSettings();
          ensureOpencodeCliEnv({ localOnly: true });
        } catch (error) {
          console.warn('[lifecycle] opencode binary pre-resolution failed:', error?.message ?? error);
        }
  ...
  /**
   * Start a managed OpenCode instance on demand (used by desktop first-launch
   * when the user picks a local source and clicks Start in the onboarding screen).
   * Safe to call when bootstrapOpenCodeAtStartup skipped starting OpenCode due to
   * OPENCHAMBER_AWAIT_USER_CHOICE=1. If a managed instance is already running or
   * an external server is in use, this is a no-op.
   *
   * Returns { started: boolean, ready: boolean, openCodePort: number | null }.
   */
  const startManagedOpenCode = async (options) => {
    const source = options?.source;

    if (state.isShuttingDown) {
      throw new Error('OpenChamber is shutting down');
    }
    if (state.isOpenCodeReady && state.openCodeProcess && !state.isExternalOpenCode) {
      return { started: false, ready: true, openCodePort: state.openCodePort };
    }
    if (state.isExternalOpenCode) {
      return { started: false, ready: state.isOpenCodeReady, openCodePort: state.openCodePort };
    }

    if (source === 'local') {
      clearResolvedOpenCodeBinary();
      await applyOpencodeBinaryFromSettings({ strict: true });
      const localBinary = ensureOpencodeCliEnv({ localOnly: true });
      if (!localBinary) {
        throw new Error('No local OpenCode installation found. Install OpenCode or use the Bundled option.');
      }
    }

    if (source === 'bundled') {
      const bundledPath = typeof resolveBundledOpenCodeCliPath === 'function'
        ? resolveBundledOpenCodeCliPath()
        : null;
      if (!bundledPath) {
        throw new Error('Bundled OpenCode CLI is not available in this build.');
      }
      process.env.OPENCODE_BINARY = bundledPath;
      clearResolvedOpenCodeBinary();
      state.resolvedOpencodeBinary = bundledPath;
    ...
    return { started: true, ready: state.isOpenCodeReady, openCodePort: state.openCodePort };
  };
