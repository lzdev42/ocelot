export const registerOpenChamberRoutes = (app, dependencies) => {
  ...
  // On-demand startup of the managed OpenCode instance. Used by the desktop
  // first-launch onboarding flow when OPENCHAMBER_AWAIT_USER_CHOICE=1 caused
  // bootstrapOpenCodeAtStartup to skip auto-starting OpenCode. The renderer
  // calls this after the user picks a source (local/bundled) and clicks Start.
  app.post('/api/opencode/start', async (req, res) => {
    try {
      if (typeof startManagedOpenCode !== 'function') {
        res.status(501).json({ error: 'Managed OpenCode start is not available in this runtime' });
        return;
      }
      const source = typeof req.body?.source === 'string' ? req.body.source : undefined;
      const result = await startManagedOpenCode({ source });
      res.json(result);
    } catch (error) {
      console.error('Failed to start managed OpenCode:', error);
      res.status(500).json({
        error: error instanceof Error ? error.message : 'Failed to start managed OpenCode',
      });
    }
  });
};
