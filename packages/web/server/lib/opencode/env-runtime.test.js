...
const originalBundledOpencodeCliDir = process.env.OCELOT_BUNDLED_OPENCODE_CLI_DIR;
const originalResourcesPath = process.resourcesPath;
const originalWslBinary = process.env.WSL_BINARY;
const originalOpenChamberWslBinary = process.env.OCELOT_WSL_BINARY;
...
afterEach(() => {
  ...
  if (typeof originalBundledOpencodeCliDir === 'string') {
    process.env.OCELOT_BUNDLED_OPENCODE_CLI_DIR = originalBundledOpencodeCliDir;
  } else {
    delete process.env.OCELOT_BUNDLED_OPENCODE_CLI_DIR;
  }
  ...
  if (typeof originalOpenChamberWslBinary === 'string') {
    process.env.OCELOT_WSL_BINARY = originalOpenChamberWslBinary;
  } else {
    delete process.env.OCELOT_WSL_BINARY;
  }
});
...
describe('OpenCode env runtime', () => {
  ...
  it('prefers a user-installed OpenCode from PATH over the bundled CLI', () => {
    ...
    process.env.OCELOT_BUNDLED_OPENCODE_CLI_DIR = bundledDir;
  ...
  it('rejects WSL settings in strict mode', async () => {
    ...
    process.env.OCELOT_WSL_BINARY = path.join(dir, 'missing-openchamber-wsl.exe');
