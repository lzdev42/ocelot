...
describe('settings helpers', () => {
  ...
  it('includes transient desktop LAN access runtime status in desktop settings response', () => {
    const helpers = createTestHelpers();
    const previousRuntime = process.env.OCELOT_RUNTIME;
    const previousActive = process.env.OCELOT_DESKTOP_LAN_ACCESS_ACTIVE;
    const previousReason = process.env.OCELOT_DESKTOP_LAN_ACCESS_BLOCKED_REASON;
    try {
      process.env.OCELOT_RUNTIME = 'desktop';
      process.env.OCELOT_DESKTOP_LAN_ACCESS_ACTIVE = 'false';
      ...
      if (typeof previousRuntime === 'string') process.env.OCELOT_RUNTIME = previousRuntime;
      else delete process.env.OCELOT_RUNTIME;
      if (typeof previousActive === 'string') process.env.OCELOT_DESKTOP_LAN_ACCESS_ACTIVE = previousActive;
      else delete process.env.OCELOT_DESKTOP_LAN_ACCESS_ACTIVE;
      if (typeof previousReason === 'string') process.env.OCELOT_DESKTOP_LAN_ACCESS_BLOCKED_REASON = previousReason;
      else delete process.env.OCELOT_DESKTOP_LAN_ACCESS_BLOCKED_REASON;
    }
  });
