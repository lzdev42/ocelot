...
describe('normalizeCustomOpenAIBaseURL', () => {
  const originalRuntime = process.env.OCELOT_RUNTIME;
  const originalAllowRemote = process.env.OCELOT_ALLOW_REMOTE_OPENAI_COMPAT_URLS;

  afterEach(() => {
    // Restore env vars after each test
    if (originalRuntime === undefined) {
      delete process.env.OCELOT_RUNTIME;
    } else {
      process.env.OCELOT_RUNTIME = originalRuntime;
    }
    if (originalAllowRemote === undefined) {
      delete process.env.OCELOT_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
    } else {
      process.env.OCELOT_ALLOW_REMOTE_OPENAI_COMPAT_URLS = originalAllowRemote;
    }
  });

  it('rejects remote URLs when OCELOT_RUNTIME is not set (web)', () => {
    delete process.env.OCELOT_RUNTIME;
    delete process.env.OCELOT_ALLOW_REMOTE_OPENAI_COMPAT_URLS;
  ...
  it('allows remote URLs when OCELOT_RUNTIME is desktop', () => {
  ...
  it('allows remote URLs when OCELOT_ALLOW_REMOTE_OPENAI_COMPAT_URLS is true', () => {
    delete process.env.OCELOT_RUNTIME;
    process.env.OCELOT_ALLOW_REMOTE_OPENAI_COMPAT_URLS = 'true';
  ...
  it('denies remote URLs on desktop when env var is explicitly false', () => {
    process.env.OCELOT_RUNTIME = 'desktop';
    process.env.OCELOT_ALLOW_REMOTE_OPENAI_COMPAT_URLS = 'false';
