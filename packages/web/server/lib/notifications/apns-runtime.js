...
//     for self-hosters who set OCELOT_APNS_* and OCELOT_PUSH_RELAY_DISABLED=true.
...
export const createApnsRuntime = (deps) => {
  ...
  const resolveApnsConfig = async () => {
    let keyId = trimmedEnv('OCELOT_APNS_KEY_ID');
    let teamId = trimmedEnv('OCELOT_APNS_TEAM_ID');
    let bundleId = trimmedEnv('OCELOT_APNS_BUNDLE_ID');
    let environment = (trimmedEnv('OCELOT_APNS_ENVIRONMENT') || '').toLowerCase();
    let p8 = normalizePem(process.env.OCELOT_APNS_P8 || '');

    const p8Path = trimmedEnv('OCELOT_APNS_P8_PATH');
    if (!p8 && p8Path) {
      try {
        p8 = (await fsPromises.readFile(p8Path, 'utf8')).trim();
      } catch (error) {
        console.warn('[APNs] Failed to read OCELOT_APNS_P8_PATH:', error?.message ?? error);
      }
    }
  ...
  // is the fallback for self-hosters who set OCELOT_APNS_* and disable the relay.
  const resolveRelayConfig = () => {
    if (trimmedEnv('OCELOT_PUSH_RELAY_DISABLED') === 'true') return null;
    const url = trimmedEnv('OCELOT_PUSH_RELAY_URL') || DEFAULT_RELAY_URL;
    return {
      url,
      registerUrl: url.replace(/\/send$/, '/register-token'),
      environment:
        (trimmedEnv('OCELOT_APNS_ENVIRONMENT') || 'sandbox').toLowerCase() === 'production'
          ? 'production'
          : 'sandbox',
    };
  };
  ...
  const sendViaDirectApns = async (deviceTokens, payload) => {
    ...
    if (!config) {
      if (!warnedUnconfigured) {
        ...
        console.warn(
          '[APNs] Relay disabled and no direct config; set OCELOT_APNS_KEY_ID / OCELOT_APNS_TEAM_ID / OCELOT_APNS_P8 for direct send.',
        );
      }
      return;
    }
