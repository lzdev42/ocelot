...
afterEach(() => {
  vi.unstubAllGlobals();
  delete process.env.OCELOT_PUSH_RELAY_URL;
  delete process.env.OCELOT_PUSH_RELAY_DISABLED;
});
...
describe('apns runtime relay mode (default)', () => {
  it('registers tokens (signed) and posts signed generic text, dropping dead tokens', async () => {
    ...
    process.env.OCELOT_PUSH_RELAY_URL = 'https://relay.test/v1/push/send';
...
describe('apns runtime direct fallback (relay disabled)', () => {
  it('signs an ES256 JWT and sends over http2 when relay is disabled', async () => {
    process.env.OCELOT_PUSH_RELAY_DISABLED = 'true';
    const targeted = [];
    const http2 = {
      connect: () => ({
        on: () => {},
