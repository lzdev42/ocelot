...
export const createPushRuntime = (deps) => {
  ...
  const resolveVapidSubject = async () => {
    const configured = process.env.OCELOT_VAPID_SUBJECT;
    if (typeof configured === 'string' && configured.trim().length > 0) {
      return configured.trim();
    }

    const originEnv = process.env.OCELOT_PUBLIC_ORIGIN;
  ...
  const ensurePushInitialized = async () => {
    ...
    if (subject === 'mailto:openchamber@localhost') {
      console.warn('[Push] No public origin configured for VAPID; set OCELOT_VAPID_SUBJECT or enable push once from a real origin.');
    }
