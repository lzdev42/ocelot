...
const createCommandTimeoutMs = () => {
  const raw = Number(process.env.OCELOT_FS_EXEC_TIMEOUT_MS);
...
const createGitReadCacheTtlMs = () => {
  const raw = Number(process.env.OCELOT_GIT_READ_CACHE_TTL_MS);
  if (Number.isFinite(raw) && raw >= 0) return raw;
  return 30 * 1000;
};

const createGitCheckIgnoreTimeoutMs = () => {
  const raw = Number(process.env.OCELOT_GIT_CHECK_IGNORE_TIMEOUT_MS);
  if (Number.isFinite(raw) && raw >= 0) return raw;
  return 2500;
};
