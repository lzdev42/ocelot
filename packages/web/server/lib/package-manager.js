...
const UPDATE_CHECK_URL = process.env.OCELOT_UPDATE_API_URL || 'https://api.openchamber.dev/v1/update/check';
...
function getOpenChamberConfigDir() {
  ...
  return path.join(os.homedir(), '.config', 'ocelot');
}
...
export function detectPackageManagerDetails() {
  ...
  const forcedPm = process.env.OCELOT_PACKAGE_MANAGER?.trim();
