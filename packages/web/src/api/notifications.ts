import type { NotificationPayload, NotificationsAPI } from '@ocelot/ui/lib/api/types';
...
const notifyWithServiceWorker = async (payload?: NotificationPayload): Promise<boolean> => {
  ...
  try {
    await registration.showNotification(payload?.title ?? 'Ocelot', {
      body: payload?.body,
      tag: payload?.tag,
    });
    return true;
...
const notifyWithWebAPI = async (payload?: NotificationPayload): Promise<boolean> => {
  ...
  try {
    ...
    new Notification(payload?.title ?? 'Ocelot', {
      body: payload?.body,
      tag: payload?.tag,
    });
    return true;
