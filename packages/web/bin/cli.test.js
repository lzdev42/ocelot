...
async function withTempOpenChamberDataDir(fn) {
  const previous = process.env.OCELOT_DATA_DIR;
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'openchamber-cli-test-'));
  process.env.OCELOT_DATA_DIR = dir;
  try {
    return await fn(dir);
  } finally {
    if (typeof previous === 'string') {
      process.env.OCELOT_DATA_DIR = previous;
    } else {
      delete process.env.OCELOT_DATA_DIR;
    }
...
describe('network-exposed auth validation', () => {
  ...
  it('allows explicit unsafe LAN override from process env only', () => {
    const previous = process.env.OCELOT_ALLOW_UNAUTHENTICATED_LAN;
    process.env.OCELOT_ALLOW_UNAUTHENTICATED_LAN = 'true';
    try {
      expect(() => assertAuthenticatedNetworkExposure({ host: '0.0.0.0' })).not.toThrow();
    } finally {
      if (typeof previous === 'string') {
        process.env.OCELOT_ALLOW_UNAUTHENTICATED_LAN = previous;
      } else {
        delete process.env.OCELOT_ALLOW_UNAUTHENTICATED_LAN;
      }
    }
  });
});
...
describe('serve host resolution', () => {
  it('uses OCELOT_HOST when --host is not provided', () => {
    const previous = process.env.OCELOT_HOST;
    process.env.OCELOT_HOST = '192.0.2.20';
    try {
      expect(resolveServeHost(undefined)).toBe('192.0.2.20');
    } finally {
      if (typeof previous === 'string') {
        process.env.OCELOT_HOST = previous;
      } else {
        delete process.env.OCELOT_HOST;
      }
    }
  });

  it('prefers explicit --host over OCELOT_HOST', () => {
    const previous = process.env.OCELOT_HOST;
    process.env.OCELOT_HOST = '192.0.2.20';
    try {
...
describe('isOpenchamberCmdline', () => {
  it('accepts OpenChamber CLI and daemon cmdlines', () => {
    expect(isOpenchamberCmdline('node /x/@ocelot/web/bin/cli.js serve')).toBe(true);
    expect(isOpenchamberCmdline('node /x/@ocelot/web/server/index.js --port 9090')).toBe(true);
