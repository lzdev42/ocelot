...
async function serveCommand(options) {
    ...
    if (!effectiveUiPassword && !options.suppressUiPasswordWarning) {
      ...
      const warningLine = 'OCELOT_UI_PASSWORD is not set';
      const warningDetail = networkExposed
        ? `server is bound to ${bindHost} and reachable on your network with no UI auth. `
          + 'Set --ui-password or OCELOT_UI_PASSWORD before exposing it over LAN.'
        : 'browser UI is unsecured. Use --ui-password or OCELOT_UI_PASSWORD.';
      if (showOutput) {
    ...
    if (options.foreground) {
      ...
      if (effectiveUiPassword) {
        process.env.OCELOT_UI_PASSWORD = effectiveUiPassword;
      }
      process.env.OCELOT_HOST = effectiveHost;
      process.env.OCELOT_RUNTIME = 'web';
    ...
    const child = spawn(runtimeBin, serverArgs, {
      ...
      env: {
        ...process.env,
        OCELOT_PORT: String(targetPort),
        OCELOT_RUNTIME: 'web',
        OPENCODE_BINARY: opencodeBinary,
        OCELOT_HOST: effectiveHost,
        ...(effectiveUiPassword ? { OCELOT_UI_PASSWORD: effectiveUiPassword } : {}),
        ...(options.apiOnly === true ? { OCELOT_API_ONLY: 'true' } : {}),
        ...(process.env.OPENCODE_SKIP_START ? { OCELOT_SKIP_OPENCODE_START: process.env.OPENCODE_SKIP_START } : {}),
      },
    });
