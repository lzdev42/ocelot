...
function parseArgs(argv = process.argv.slice(2)) {
  ...
  const options = {
    port: DEFAULT_PORT,
    host: undefined,
    uiPassword: process.env.OCELOT_UI_PASSWORD || undefined,
    json: false,
    all: false,
    follow: true,
...
ENVIRONMENT:
  OCELOT_HOST             Bind address (e.g. 0.0.0.0 for all interfaces)
  OCELOT_UI_PASSWORD      Alternative to --ui-password flag
  OCELOT_API_ONLY         Set to true/1 to start API routes only
  OCELOT_DATA_DIR         Override OpenChamber data directory
  ...
  OCELOT_OPENCODE_HOSTNAME  Bind hostname for managed OpenCode server (default: 127.0.0.1)
