...
function collectStartupEnv(options = {}) {
  ...
  if (uiPassword) {
    env.OCELOT_UI_PASSWORD = uiPassword;
  }
  if (options.apiOnly === true) {
    env.OCELOT_API_ONLY = 'true';
  }
  if (typeof process.env.OCELOT_DATA_DIR === 'string' && process.env.OCELOT_DATA_DIR.trim().length > 0) {
    env.OCELOT_DATA_DIR = path.resolve(process.env.OCELOT_DATA_DIR.trim());
  }
  return env;
}
