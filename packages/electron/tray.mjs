...
//
// Surfaces a glanceable, always-visible view of Ocelot's live state:
...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'Ocelot — no active sessions';
  const bits = [];
  ...
  return `Ocelot — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    template.push(
      ...
      { label: 'Show Ocelot', click: () => onAction({ type: 'show-main-window' }) },
      { type: 'separator' },
      { label: 'Quit Ocelot', click: () => onAction({ type: 'quit' }) },
    );
