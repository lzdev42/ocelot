...
if [[ -n "$OCELOT_SSH_ASKPASS_VALUE" ]]; then
  if [[ "$PROMPT" == *"assword"* || "$PROMPT" == *"passphrase"* ]]; then
    printf '%s\\n' "$OCELOT_SSH_ASKPASS_VALUE"
    exit 0
  fi
fi
...
const waitLocalForwardReady = async (localPort) => {
  ...
  throw new Error('Timed out waiting for forwarded Ocelot health');
};
...
export class ElectronSshManager {
  ...
  async issueClientToken(localUrl, openchamberPassword) {
    ...
    const loginResponse = await fetch(new URL('/auth/session', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({
        ...
        clientLabel: 'Ocelot Desktop SSH',
      }),
    });
    if (!loginResponse.ok) {
      throw new Error(`Configured Ocelot UI password was rejected by forwarded server (status ${loginResponse.status})`);
    }
    ...
    const tokenResponse = await fetch(new URL('/api/client-auth/clients', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({ label: 'Ocelot Desktop SSH' }),
    });
  ...
  async spawnMasterProcess(parsed, controlPath, askpassPath, sshPassword) {
    const child = spawn('ssh', buildSshArgs(parsed, [
      ...
      env: {
        ...
        DISPLAY: '1',
        ...(sshPassword ? { OCELOT_SSH_ASKPASS_VALUE: sshPassword.trim() } : {}),
      },
    });
    return child;
  }
  ...
  configuredOcelotPassword(instance) {
  ...
  async currentRemoteOcelotVersion(parsed, controlPath) {
    try {
  ...
  async installOcelotManaged(parsed, controlPath, version, preferred) {
    ...
    if (preferred === 'bun') {
      if (hasBun) commands.push(`bun add -g @ocelot/web@${version}`);
      if (hasNpm) commands.push(`npm install -g @ocelot/web@${version}`);
    ...
    throw lastError || new Error('Failed to install Ocelot on remote host');
  }
    ...
    if (isLivenessHttpStatus(infoStatus)) {
      if (isAuthHttpStatus(infoStatus)) {
        if (openchamberPassword && authStatus !== 200) {
          throw new Error(`Remote Ocelot requires UI authentication and configured password was rejected (auth status ${authStatus})`);
        }
        if (isLivenessHttpStatus(healthStatus)) return {};
        throw new Error('Remote Ocelot requires UI authentication on /api/system/info; configure Ocelot UI password');
      }
    } else if (isLivenessHttpStatus(healthStatus)) {
      return {};
    } else {
      throw new Error(`Remote Ocelot probe failed (info status ${infoStatus}, health status ${healthStatus})`);
    }
  ...
  async startRemoteServerManaged(parsed, controlPath, instance, desiredPort) {
    let envPrefix = 'OCELOT_RUNTIME=ssh-remote';
    const secret = this.configuredOcelotPassword(instance);
    if (secret) {
      envPrefix += ` OCELOT_UI_PASSWORD=${shellQuote(secret)}`;
    }
  ...
  async ensureRemoteServer(instance, parsed, controlPath) {
    if (instance.remoteOpenchamber.mode === 'external') {
      if (!instance.remoteOpenchamber.preferredPort) {
        throw new Error('External mode requires a preferred remote Ocelot port');
      }
      const port = instance.remoteOpenchamber.preferredPort;
      this.setStatus(instance.id, 'server_detecting', 'Probing external Ocelot server', null, null, port, false, 0, false);
      await this.probeRemoteSystemInfo(parsed, controlPath, port, this.configuredOcelotPassword(instance));
      return { remotePort: port, startedByUs: false };
    }

    this.setStatus(instance.id, 'remote_probe', 'Checking remote Ocelot installation');
    const installedVersion = await this.currentRemoteOcelotVersion(parsed, controlPath);
    if (!installedVersion) {
      this.setStatus(instance.id, 'installing', 'Installing Ocelot on remote host');
      await this.installOcelotManaged(parsed, controlPath, this.appVersion, instance.remoteOpenchamber.installMethod);
    } else if (installedVersion !== this.appVersion) {
      this.setStatus(instance.id, 'updating', `Updating remote Ocelot from ${installedVersion} to ${this.appVersion}`);
      await this.installOcelotManaged(parsed, controlPath, this.appVersion, instance.remoteOpenchamber.installMethod);
    }

    this.setStatus(instance.id, 'server_detecting', 'Detecting managed Ocelot server');
    let remotePort = instance.remoteOpenchamber.preferredPort || null;
    let startedByUs = false;
    if (remotePort && !(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredOcelotPassword(instance)))) {
      remotePort = null;
    }
    if (!remotePort) {
      this.setStatus(instance.id, 'server_starting', 'Starting managed Ocelot server');
    ...
    if (!(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredOcelotPassword(instance)))) {
      throw new Error('Managed Ocelot server failed to become reachable');
    }
  ...
  async connectBlocking(instance) {
    ...
    const clientToken = await this.issueClientToken(localUrl, this.configuredOcelotPassword(instance));
