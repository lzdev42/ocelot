...
const configPath = path.join(os.homedir(), '.config', 'ocelot', 'oc-dev.json');
...
function startInstalledInstance(directory, port) {
  ...
  run('node', [cliPath, '--port', port], {
    cwd: directory,
    env: {
      OCELOT_UI_PASSWORD: process.env.OCELOT_PASSWORD || '',
      OCELOT_HOST: '0.0.0.0',
    },
...
async function deployWeb(options, config) {
  ...
  step('Removing old global package', () => {
    run('bun', ['remove', '-g', '@ocelot/web'], { allowFail: true, label: 'remove @ocelot/web' });
  ...
  step(`Starting global instance on ${GLOBAL_PORT}`, () => {
    ...
    run('node', [cliPath, '--port', GLOBAL_PORT], { env: { OCELOT_UI_PASSWORD: process.env.OCELOT_PASSWORD || '', OCELOT_HOST: '0.0.0.0' } });
  });
}
...
async function deployRemoteWeb(options, config) {
  ...
  step(`Stopping remote instance on ${host}:${port}`, () => run('ssh', [host, `set -e; ${REMOTE_RUNTIME_ENV}; cd ~/${dir} 2>/dev/null || exit 0; PORT=${quote(port)}; TMPDIR=$(node -p "require('os').tmpdir()" 2>/dev/null || echo /tmp); PIDFILE="$TMPDIR/openchamber-${port}.pid"; INSTANCEFILE="$TMPDIR/openchamber-${port}.json"; if [ -f ./node_modules/@ocelot/web/bin/cli.js ]; then bun ./node_modules/@ocelot/web/bin/cli.js stop --port "$PORT" >/dev/null 2>&1 || node ./node_modules/@ocelot/web/bin/cli.js stop --port "$PORT" >/dev/null 2>&1 || true; fi; if command -v lsof >/dev/null 2>&1; then lsof -ti :"$PORT" | xargs -r kill >/dev/null 2>&1 || true; sleep 0.5; lsof -ti :"$PORT" | xargs -r kill -9 >/dev/null 2>&1 || true; fi; rm -f "$PIDFILE" "$INSTANCEFILE"`], { label: 'stop remote instance' }));
  ...
  step(`Starting remote instance on ${host}:${port}`, () => run('ssh', [host, `set -e; cd ~/${dir}; ${REMOTE_RUNTIME_ENV}; PASSWORD_VALUE=$(grep '^export OCELOT_UI_PASSWORD=' ~/.bashrc 2>/dev/null | sed -E 's/.*=["“]?([^"”]+)["”]?/\\1/' || true); if [ -n "$PASSWORD_VALUE" ]; then export OCELOT_UI_PASSWORD="$PASSWORD_VALUE"; fi; if [ ${quote(apiOnly)} = 'true' ]; then export OCELOT_API_ONLY=true; fi; OCELOT_HOST=0.0.0.0 node ./node_modules/@ocelot/web/bin/cli.js --port ${quote(port)} >/dev/null 2>&1; sleep 0.5; if command -v lsof >/dev/null 2>&1; then lsof -ti :${quote(port)} >/dev/null 2>&1 || exit 1; fi`]));
...
async function startWebDev(options) {
  ...
  if (mode === 'hmr-lan') {
    ...
    run('bun', ['run', 'dev:web:hmr'], { env: { OCELOT_HMR_HOST: '0.0.0.0' } });
  } else if (mode === 'full') {
...
async function startMobileDev(options) {
  ...
  const hmrPort = process.env.OCELOT_HMR_UI_PORT || '5180';
  ...
  const devServer = spawn('bun', ['x', 'vite', '--config', 'local-dev-mobile-vite.config.mjs', '--host', hmrBindHost, '--port', hmrPort, '--strictPort'], {
    cwd: repoRoot,
    stdio: 'inherit',
    env: { ...process.env, OCELOT_DISABLE_PWA_DEV: '1' },
  });
...
function prepareOpenCodeCli() {
  step('Preparing bundled OpenCode CLI', () => run('bun', ['--filter', '@ocelot/electron', 'prepare:opencode-cli']));
}
