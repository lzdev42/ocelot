...
# alias; OCELOT_UI_PASSWORD is the canonical runtime variable.
if [ -z "${OCELOT_UI_PASSWORD:-}" ] && [ -n "${UI_PASSWORD:-}" ]; then
  OCELOT_UI_PASSWORD="$UI_PASSWORD"
  export OCELOT_UI_PASSWORD
fi

if [ -n "${OCELOT_UI_PASSWORD:-}" ]; then
...
OCELOT_HOST="${OCELOT_HOST:-0.0.0.0}"
export OCELOT_HOST
...
if [ -n "${OCELOT_UI_PASSWORD:-}" ]; then
  set -- "$@" --ui-password "$OCELOT_UI_PASSWORD"
fi
"$@"
